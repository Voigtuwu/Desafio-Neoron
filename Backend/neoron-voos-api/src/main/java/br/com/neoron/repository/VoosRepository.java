...
public interface VoosRepository extends JpaRepository<Voos, UUID>, VoosRepositoryCustom {
	...
	boolean existsConflictingFlights(@Param("origem") Aeroportos origem, @Param("dataPartida") LocalDate dataPartida,
			@Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime);

	@Query("SELECT COUNT(v) > 0 FROM Voos v WHERE v.origem = :origem " + "AND v.destino = :destino "
			+ "AND v.dataPartida = :dataPartida " + "AND v.id != :id " + "AND (v.horaPartida < :endTime "
			+ "AND v.horaChegada > :startTime)")
