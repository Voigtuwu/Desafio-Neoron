...
public interface VoosRepository extends JpaRepository<Voos, UUID>, VoosRepositoryCustom {
    ...
    @Query("SELECT COUNT(v) > 0 FROM Voos v WHERE v.origem = :origem " +
            ...
            "AND (v.horaPartida < :endTime " +
            "AND v.horaChegada > :startTime)")
    boolean existsConflictingFlights(
            @Param("origem") Aeroportos origem,
    ...
    @Query("SELECT COUNT(v) > 0 FROM Voos v WHERE v.origem = :origem " +
            ...
            "AND v.dataPartida = :dataPartida")
    boolean existsSameOriginAndDestinationOnSameDay(
            @Param("origem") Aeroportos origem,
            @Param("destino") Aeroportos destino,
            @Param("dataPartida") LocalDate dataPartida
    );
}
